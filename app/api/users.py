...
from app.schema.user_schema import UserCreate, UserListResponse, UserResponse, UserUpdate
...
async def create_user(
    ...
    **Permisos:** Solo ADMIN y COORDINADOR_LOGISTICA pueden crear usuarios
    ...
    if current_user["rol"] not in [UserRole.ADMIN.value, UserRole.COORDINADOR_LOGISTICA.value]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="No tienes permisos para crear usuarios"
        )
...
async def list_users(
    ...
    **Permisos:** Requiere estar autenticado
    ...
    admin_roles = [UserRole.ADMIN.value, UserRole.COORDINADOR_LOGISTICA.value]
...
async def update_user(
    ...
    **Permisos:** Solo ADMIN puede actualizar cualquier usuario.
    Otros pueden actualizar su propio perfil
    """
    # Verificar permisos
    if current_user["rol"] != UserRole.ADMIN.value and current_user["user_id"] != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Solo puedes actualizar tu propio perfil"
        )
